...
model = AutoModelForSequenceClassification.from_pretrained(
    ...
    num_labels=1).to(device)  # copy cpu default model, send to GPU
...
# BEGIN MODEL_ID LOOP
# Loop once per theta
thetas = -np.sort(-np.arange(-3, 3.1, .5))[:2]
model_id = int(0)
...
# ----------------------------------------------------------------------------------------------------------------------
# SAVE DATA
out_fit.to_csv(path_or_buf="output/Separate_out_fit_test.csv", index=False)
out_loss.to_csv(path_or_buf="output/Separate_out_loss_test.csv", index=False)
